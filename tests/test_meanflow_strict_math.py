...
class PolynomialVelocityField(nn.Module):
    ...
    def forward(self, unified_input):
        ...
        # Check if 2D (B, 4) where last two dims are time inputs
        if unified_input.dim() == 2 and unified_input.shape[1] == 4:
            z = unified_input[:, :2]
            t = unified_input[:, 2:3]
            ...
            raise NotImplementedError("Only implemented for (B, 4) input")
...
class TestStrictMeanFlowMath:
    def test_polynomial_field_jvp_match(self):
        ...
        assert torch.allclose(v_t_out, v_t_analytical, atol=1e-6), (
            "Forward pass mismatch"
        )

        jvp_analytical = torch.cat([z1 * (1 + t**2), 2 * (z2**3)], dim=1)
    ...
    def test_meanflow_target_match(self):
        ...
        assert torch.allclose(u_tgt_auto, u_tgt_analytical, atol=1e-5), (
            "Target mismatch"
        )
    ...
    def test_straight_path_invariance(self):
        ...
        assert torch.allclose(jvp_out, torch.zeros_like(jvp_out)), (
            "JVP for constant field should be 0"
        )

        u_tgt = v_t - (t - r) * jvp_out
        assert torch.allclose(u_tgt, velocity), (
            "Target for constant field should be velocity itself"
        )
